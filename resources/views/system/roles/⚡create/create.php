...
new class extends Component
{
    ...
    public function save()
    {
        ...
        if (in_array('super admin', $this->permissions_selecteds) && !auth()->user()->hasRole('sudo')) {
            $this->addError('permissions_selecteds', __('system.roles.create.super_admin_permission_error'));
            return null;
        }

        $sluggedName = Str::slug($this->display_name);
        if (Role::where('name', $sluggedName)->exists()) {
            $this->addError('display_name', __('system.roles.create.name_already_exists'));
            return null;
       }
